...
- `assignment.pdf` -- empty assignment PDF so the build script works, replace this with your assignment
...
If you ever need to go edit `template/` to add something you think should be included by default, create an issue or a PR.
...
Or use the build script to merge the assignment PDF (recommended):
...
Note that you might need to install [Ghostscript](https://www.ghostscript.com/index.html) or [`pdftk`](https://www.pdflabs.com/tools/pdftk-the-pdf-toolkit/)
to run the compile script if you don't have them. (Both should be available in most package managers.)
